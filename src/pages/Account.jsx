...
export default function Account() {
  ...
  const fetchAllLogs = async () => {
    ...
    try {
      ...
      console.error('fetchAllLogs:', err)
    }
  }
  ...
  const handleExportPDF = async () => {
    if (allLogs.length === 0) await fetchAllLogs()
    setExporting(true)
    try {
  ...
  // Use profile values (kept accurate by recalcTotalMinutes in Dashboard)
  const totalMins   = profile?.totalMinutes || 0
  const totalHours  = Math.floor(totalMins / 60)
  const daysStudied = profile?.daysStudied  || new Set(allLogs.map(l=>l.date).filter(Boolean)).size
  const avgMins     = daysStudied > 0 ? Math.round(totalMins / daysStudied) : 0
      ...
      <div className="page-container pt-6">
        ...
        <div className="card mb-4">
          <div className="flex items-center gap-4 mb-4">
            <img
              src={user?.photoURL || `https://ui-avatars.com/api/?name=${encodeURIComponent(profile?.name||'User')}&background=0ea5e9&color=fff&size=80`}
              alt="avatar"
              className="w-16 h-16 rounded-2xl border-2 border-brand-500/40 flex-shrink-0 object-cover"
              onError={e => { e.target.src = `https://ui-avatars.com/api/?name=User&background=0ea5e9&color=fff&size=80` }}
            />
          ...
          {editing ? (
            <div className="space-y-3">
              <div>
                ...
                <input type="text" value={form.college}
                  ...
                  className="input-field" placeholder="Your college" />
              </div>
              <div>
                  ...
                  {GROUPS.map(g => (
                    <button key={g} onClick={() => setForm(f=>({...f, group:g}))}
                      className={`py-2.5 rounded-xl text-xs font-display border transition-all ${
                        form.group===g ? 'bg-brand-500 border-brand-400 text-white' : 'bg-surface-700 border-white/10 text-white/50'
                      }`}>
                  ...
                  {GOALS.map(g => (
                    <button key={g} onClick={() => setForm(f=>({...f, dailyGoal:g}))}
                      className={`w-11 h-9 rounded-lg text-xs font-display font-semibold border transition-all ${
                        form.dailyGoal===g ? 'bg-accent-500 border-accent-400 text-white' : 'bg-surface-700 border-white/10 text-white/50'
                      }`}>
                      {g}h
                    </button>
                  ))}
              ...
              <button onClick={handleSave} disabled={saving} className="btn-primary w-full mt-1">
                {saving
            ...
            <div className="grid grid-cols-2 gap-2">
              {[
                { label: t('onboarding.college'), value: profile?.college || '—' },
                { label: t('account.goal'),       value: `${profile?.dailyGoal || 8}h / day`, accent: true },
                { label: t('account.joined'),     value: joinedDate },
                { label: 'Days Studied',          value: `${daysStudied}`, green: true },
                ...
                <div key={label} className="bg-surface-700 rounded-xl p-3">
                  ...
                  <p className={`font-display font-semibold text-xs truncate ${accent ? 'text-accent-400' : green ? 'text-green-400' : 'text-white'}`}>
                    {value}
                  </p>
                </div>
              ))}
