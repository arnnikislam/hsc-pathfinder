...
export default function Leaderboard() {
  ...
  const getDateFilter = () => {
    ...
    return '2026-01-01' // all time from start of year
  }
  ...
  const fetchLeaderboard = async () => {
    ...
    try {
      ...
      if (Object.keys(userMinutes).length === 0) {
        setEntries([])
        setLoading(false)
        return
      }
      ...
      let board = Object.entries(userMinutes)
        ...
        .filter(e => e.name) // only users with profiles
  ...
  const groupLabel = (g) => {
    const map = { science: '🔬', arts: '🎨', commerce: '💼' }
    return map[g] || ''
  }
  ...
  return (
    <div className="min-h-screen bg-surface-900">
      <div className="page-container pt-6">
        ...
        {/* Time tabs */}
        <div className="flex gap-1.5 mb-3 bg-surface-800 p-1 rounded-xl">
          {TABS.map(tab => (
            <button
              key={tab}
        ...
        <div className="flex gap-2 mb-4 overflow-x-auto no-scrollbar pb-1">
          {GROUPS.map(g => (
            <button
              ...
              className={`flex-shrink-0 px-3 py-1.5 rounded-full text-xs font-display font-medium transition-all duration-200 border ${
                activeGroup === g
        ...
        {loading ? (
          ...
          <div className="text-center py-16">
            ...
            <p className={`text-white/30 text-sm ${isBn ? 'font-bengali' : ''}`}>
              {t('leaderboard.empty')}
            </p>
          </div>
        ) : (
          <div className="space-y-2">
            {entries.map((entry, idx) => {
              ...
              return (
                <div
                  key={entry.uid}
                  className={`flex items-center gap-3 rounded-2xl px-4 py-3 transition-all duration-200 border ${
                    isMe
                      ? 'bg-brand-500/10 border-brand-500/30 neon-border'
                      : rank <= 3
                ...
                >
                  ...
                  {/* Avatar */}
                  <img
                    src={entry.photoURL || `https://ui-avatars.com/api/?name=${entry.name}&background=0ea5e9&color=fff&size=40`}
                    alt={entry.name}
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-1.5">
                      <p className={`text-sm font-display font-semibold text-white truncate ${isMe ? 'text-brand-300' : ''}`}>
                        {entry.name} {isMe ? '(You)' : ''}
                      </p>
                      <span className="text-xs">{groupLabel(entry.group)}</span>
                    </div>
