...
import { collection, addDoc, query, where, getDocs, Timestamp, doc, updateDoc, increment, deleteDoc } from 'firebase/firestore'
...
import { Plus, Clock, Flame, TrendingUp, Target, Trash2 } from 'lucide-react'
...
export default function Dashboard() {
  ...
  // Cap percent display at 100 max
  const percent = goalMinutes > 0 ? Math.min(100, Math.round((todayMinutes / goalMinutes) * 100)) : 0
  const actualPercent = goalMinutes > 0 ? Math.round((todayMinutes / goalMinutes) * 100) : 0
  ...
  const fetchTodayLogs = async () => {
    ...
    try {
      ...
      setTodayLogs(logs)
      setTodayMinutes(logs.reduce((sum, l) => sum + l.minutes, 0))
    } catch (err) {
      console.error(err)
    } finally {
  ...
  useEffect(() => { if (user) fetchTodayLogs() }, [user])
  ...
  const handleLogStudy = async () => {
    ...
    // Max single entry = 16 hours (realistic study limit)
    if (mins > 960) { toast.error('Max 16 hours per entry'); return }

    // Prevent total daily exceeding 20h (realistic hard cap)
    if (todayMinutes + mins > 1200) {
      toast.error(`Can't exceed 20h total in a day! Already logged ${fmtMin(todayMinutes)}`)
      return
    }
  ...
  const handleDeleteLog = async (log) => {
    try {
      ...
      // Subtract from totalMinutes
      await updateDoc(doc(db, 'users', user.uid), {
        totalMinutes: increment(-log.minutes),
      })
  ...
  const dailyRemaining = Math.max(0, 1200 - todayMinutes)
    ...
    <div className="min-h-screen bg-surface-900">
      <div className="page-container pt-6">
        ...
        <div className="flex items-center justify-between mb-4 pr-16">
          ...
          <img
            src={user?.photoURL || `https://ui-avatars.com/api/?name=${encodeURIComponent(profile?.name || 'User')}&background=0ea5e9&color=fff`}
            alt="avatar" className="w-10 h-10 rounded-full border-2 border-brand-500/50"
          />
        </div>
        ...
        <div className="card mb-4">
          ...
          <div className="flex items-center gap-6">
            <div className="relative flex-shrink-0">
              <ProgressRing percent={actualPercent} size={110} stroke={9} />
            ...
            <div className="flex-1 space-y-2">
              ...
              <div>
                ...
                <p className="text-base font-display font-semibold text-accent-400">
                  {actualPercent >= 100 ? (isBn ? 'সম্পন্ন ✅' : 'Done ✅') : fmtMin(Math.max(0, goalMinutes - todayMinutes))}
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="mt-4 h-2 bg-white/5 rounded-full overflow-hidden">
            <div className="h-full rounded-full transition-all duration-700"
              style={{
                width: `${percent}%`,
                background: actualPercent >= 100 ? 'linear-gradient(90deg,#22c55e,#16a34a)' : 'linear-gradient(90deg,#0ea5e9,#f97316)'
              }} />
          </div>

          {actualPercent >= 100 && (
            <p className="text-center text-green-400 text-xs font-display mt-2">
        ...
        <div className="grid grid-cols-3 gap-3 mb-4">
          {[
            ...
            <div key={label} className="card text-center py-4">
              <p className={`text-[10px] text-white/40 mb-1 ${isBn ? 'font-bengali' : ''}`}>{label}</p>
        ...
        <div className="card mb-4">
          <div className="flex items-center justify-between mb-3">
            ...
            <div className="flex items-center gap-2">
              <span className="text-[10px] text-white/30 font-display">{fmtMin(todayMinutes)} / 20h max</span>
          ...
          {loadingLogs ? (
            ...
            <p className={`text-white/30 text-sm text-center py-4 ${isBn ? 'font-bengali' : ''}`}>{t('dashboard.no_logs')}</p>
          ) : (
              ...
              {todayLogs.map(log => (
                <div key={log.id} className="flex items-center justify-between bg-surface-700 rounded-xl px-4 py-3">
                  ...
                  <div className="flex items-center gap-3">
                    <span className="text-brand-400 font-display font-semibold text-sm">{fmtMin(log.minutes)}</span>
                    <button onClick={() => handleDeleteLog(log)} className="text-red-400/50 hover:text-red-400 transition-colors">
                      <Trash2 size={13} />
                    </button>
                  </div>
                </div>
        ...
        <div className="card mb-6 bg-accent-500/5 border border-accent-500/15">
          <p className={`text-accent-400/80 text-xs leading-relaxed ${isBn ? 'font-bengali' : ''}`}>
            📌 {isBn
              ...
              : 'Log honestly. Max 20h can be logged per day.'}
          </p>
        </div>
      </div>
      ...
      {showAddModal && (
        <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="w-full max-w-sm bg-surface-800 border border-white/10 rounded-3xl p-6 animate-slide-up">
            ...
            <p className="text-white/30 text-[11px] text-center mb-5 font-display">
              {isBn ? `আজ বাকি লগ করা যাবে: ${fmtMin(dailyRemaining)}` : `Remaining daily allowance: ${fmtMin(dailyRemaining)}`}
            </p>
            ...
            <div className="grid grid-cols-2 gap-3">
              <button onClick={() => { setShowAddModal(false); setAddHours(''); setAddMins('') }} className="btn-secondary">
