...
export default function Dashboard() {
  ...
  const percent = Math.round((todayMinutes / goalMinutes) * 100)
  ...
  const fetchTodayLogs = async () => {
    ...
    try {
      ...
      const q = query(
        ...
        where('date', '>=', format(startOfDay(today), 'yyyy-MM-dd')),
        where('date', '<=', format(endOfDay(today), 'yyyy-MM-dd')),
  ...
  const handleLogStudy = async () => {
    const mins = (parseInt(addHours) || 0) * 60 + (parseInt(addMins) || 0)
    if (mins <= 0) { toast.error('Please enter valid time'); return }
    if (mins > 720) { toast.error('Maximum 12 hours per entry'); return }
    ...
    try {
      ...
      await addDoc(collection(db, 'studyLogs'), {
        ...
        hours: parseInt(addHours) || 0,
        mins: parseInt(addMins) || 0
      })
      toast.success(`${addHours}h ${addMins}m logged! 📚`)
      setAddHours(0)
      setAddMins(0)
  ...
  const fmtMin = (m) => {
    ...
    return h > 0 ? `${h}${hs} ${min > 0 ? `${min}${ms}` : ''}`.trim() : `${min}${ms}`
  }
      ...
      <div className="page-container pt-6">
        ...
        <div className="grid grid-cols-3 gap-3 mb-4">
          {[
            ...
            { icon: TrendingUp, label: t('dashboard.total_hours'), value: `${Math.floor((profile?.totalMinutes || 0) / 60)}h`, color: 'text-brand-400' },
        ...
        <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="w-full max-w-sm bg-surface-800 border border-white/10 rounded-3xl p-6 animate-slide-up">
            ...
            <p className={`text-accent-400/70 text-xs text-center mb-4 ${isBn ? 'font-bengali' : ''}`}>
              📌 {isBn
                ? 'সততার সাথে সময় লগ করো — তোমার নিজের উন্নতির জন্য!'
                : 'Log honestly — this is for YOUR growth!'}
            </p>
