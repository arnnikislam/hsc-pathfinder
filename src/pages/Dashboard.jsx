...
// Recalculate totalMinutes from scratch by reading all logs — prevents drift from bugs
async function recalcTotalMinutes(uid) {
  ...
  const datesSet = new Set(snap.docs.map(d => d.data().date).filter(Boolean))
  await updateDoc(doc(db, 'users', uid), {
    totalMinutes: total,
    daysStudied:  datesSet.size,
  })
  return { total, daysStudied: datesSet.size }
}
...
export default function Dashboard() {
  ...
  // Use recalculated value from Firestore, not the potentially drifted one
  ...
  // On first load, recalculate totalMinutes to fix any -30h drift
  useEffect(() => {
    if (user) {
      fetchTodayLogs()
      recalcTotalMinutes(user.uid).then(() => refreshProfile())
    }
  }, [user])
  ...
  const handleLog = async () => {
    ...
    if (mins > 960)   { toast.error('Max 16h per single entry');     return }
    if (todayMinutes + mins > 1200) {
      toast.error(`Total can't exceed 20h/day. Remaining: ${fmtMinDisplay(Math.max(0, 1200 - todayMinutes), t)}`)
      return
    }
    ...
    try {
      ...
      // Recalc from scratch so totalMinutes never goes negative
      await recalcTotalMinutes(user.uid)
  ...
  const handleDelete = async (log) => {
    try {
      ...
      // Recalc from scratch after delete too
      await recalcTotalMinutes(user.uid)
      ...
      <div className="page-container pt-6">
        ...
        <div className="flex items-center justify-between mb-4 pr-14">
          ...
          <img
            ...
            alt="avatar" className="w-10 h-10 rounded-full border-2 border-brand-500/50 flex-shrink-0"
          />
        </div>
          ...
          <div className="flex items-center gap-5">
            ...
            <div className="flex-1 space-y-2">
              <div>
                ...
                <p className="text-xl font-display font-bold text-white">{fmt(todayMinutes)}</p>
              </div>
              <div>
              ...
              <div>
                ...
                <p className="text-base font-display font-semibold text-accent-400">
                  {percent >= 100
                    ? (isBn ? 'সম্পন্ন ✅' : 'Done ✅')
                    : fmt(Math.max(0, goalMinutes - todayMinutes))}
                </p>
              </div>
            </div>
          </div>
        ...
        <div className="grid grid-cols-3 gap-3 mb-4">
          {[
            ...
            { icon: Target,     label: 'Days Studied',             value: `${daysStudied}`,               color: 'text-green-400'  },
            ...
            <div className="space-y-2">
              {todayLogs.map(log => (
                <div key={log.id} className="flex items-center justify-between bg-surface-700 rounded-xl px-4 py-3">
                  ...
                  <div className="flex items-center gap-3">
                    <span className="text-brand-400 font-display font-bold text-sm">{fmt(log.minutes)}</span>
              ...
              {todayLogs.length > 1 && (
                <div className="flex justify-between border-t border-white/5 pt-2">
                  ...
                  <span className="text-white font-display font-bold text-sm">{fmt(todayMinutes)}</span>
                </div>
              )}
            </div>
          )}
        ...
        <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="w-full max-w-sm bg-surface-800 border border-white/10 rounded-3xl p-6 animate-slide-up">
            ...
            <p className="text-white/25 text-[11px] text-center mb-5 font-display">
              {isBn
                ? `আজ আরও লগ করা যাবে: ${fmt(Math.max(0, 1200 - todayMinutes))}`
                : `Remaining today: ${fmt(Math.max(0, 1200 - todayMinutes))}`}
            </p>
            ...
            {previewMins > 0 && (
              <div className="bg-brand-500/10 border border-brand-500/20 rounded-xl px-4 py-2 mb-3 text-center">
                <span className="text-brand-400 font-display font-semibold text-sm">
                  ✏️ {fmt(previewMins)} {isBn ? 'লগ হবে' : 'will be logged'}
                </span>
              </div>
            )}
            ...
            <div className="grid grid-cols-2 gap-3">
              <button onClick={() => { setShowModal(false); setAddHours(''); setAddMins('') }}
